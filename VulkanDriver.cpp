...
VulkanDriver::VulkanDriver(Window window) {}; // initialize the vulkan instance.
...
void VulkanDriver::createSurface() {
	mRenderwindow->CreateSurface(instance, mSurface);
}
...
void VulkanDriver::createSwapChainViews() {
	swapChainImageViews.resize(swapChainImages.size());

	for (size_t i = 0; i < swapChainImages.size(); i++) {
		std::runtime_error("need to make the image view");
...
VkSampleCountFlagBits VulkanDriver::getMaxUsableSampleCount() {
...
VkExtent2D VulkanDriver::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
	if (capabilities.currentExtent.width != UINT32_MAX) {
		...
		int width, height;
		mRenderwindow->getFrameBufferSize(&width, &height);
